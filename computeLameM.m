function [M_coeffs, p_values] = computeLameM(h1, h2, h3, n)
    % Calculate Lamé functions of class M
    ...
    % Construct tridiagonal matrix M symbolically
    syms p;
    M_matrix = sym(zeros(r, r));
    
    for j = 1:r
        % Main diagonal
        M_matrix(j,j) = -alpha*(p - (n - 2*j + 1)^2) + (2*n - 4*j + 4)*h3^2;
        
        % Upper diagonal (j < r)
        if j < r
            M_matrix(j,j+1) = 2*j*(2*n - 2*j + 1);
        end
        
        % Lower diagonal (j > 1)
        if j > 1
            M_matrix(j,j-1) = -beta*(n - 2*j + 2)*(n - 2*j + 1);  % Fixed index
        end
    end
    
    % Solve det(M_matrix) = 0 for p
    try
        p_solutions = solve(det(M_matrix) == 0, p, 'Real', true);
    ...
    M_coeffs = cell(length(p_values), 1);
    ...
    for i = 1:length(p_values)
        ...
        for k = 0:r-2
            numerator = (alpha*(current_p - (n - 2*k - 1)^2) - (2*n - 4*k - 1)*h3^2)*b(k+1);
            if k > 0
